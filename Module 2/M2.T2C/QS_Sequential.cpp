...
// Function to choose a pivot randomly then swap it with the last element
// Then it will call the partition function
int randomPartition(int array[], int min, int max)
{
    int random = min + rand() % (max - min); // Will get a random number between min to max
...
void quickSort(int array[], int min, int max)
{
    if (min < max)
    {
        ...
        // Quicksort using the Random partition function
        //int index = randomPartition(array, min, max);
